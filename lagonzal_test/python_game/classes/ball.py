...
class Ball:
	...
	def calculateRebound(self, paddle : Paddle, factor):
		...
		print(f"Paddle center is {paddle.yPos + paddle.height / 2} and ball center is {self.yPos}")
		print(f"Relative interesesction {relativeIntersection}")
		normalIntersec = relativeIntersection / (paddle.height / 2)
		bounceAngle = (normalIntersec * self.MAXANGLE)
		print(f"Normal intersec: {normalIntersec} and bounce Angle: {bounceAngle}")
	...
	def calculateVectors(self, bounceAngle : float, factor : int):
		...
		print(f"xVector {self.xVector} yVector {self.yVector}")
		print("=============================================================")
